...
export default function Home() {
  ...
  const [taskCategory, setTaskCategory] = useState("Personal"); // ✅ New state
  const [editingId, setEditingId] = useState<number | null>(null);
  const [editText, setEditText] = useState("");
  const [editCategory, setEditCategory] = useState("Personal"); // ✅ For editing
  ...
  useEffect(() => {
    ...
    const groupedTasks = tasks.reduce((acc: any, task: any) => {
      const category = task.category || "Uncategorized"; // default
  ...
  const handleAddTask = async () => {
    ...
    const res = await fetch("/api/tasks", {
      ...
      body: JSON.stringify({ title: taskName, category: taskCategory }), // ✅ include category
    });
  ...
  const handleEditTask = async (id: number) => {
    const res = await fetch("/api/tasks", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id, title: editText, category: editCategory }), // ✅ include category
    });
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 flex flex-col items-center py-10 px-4">
      ...
      <div className="flex flex-col gap-3 mb-10 w-full max-w-md">
        <input
          ...
          className="flex-1 px-4 py-2 bg-white border-2 border-blue-400 rounded-xl shadow-md 
                    placeholder-black text-black focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
        ...
        <select
          ...
          className="px-4 py-2 bg-white border-2 border-blue-400 rounded-xl shadow-md 
    ...
    >
      ...
      <ul className="space-y-3">
        {groupedTasks[category].map((task: any) => (
          <li
            key={task.id}
            className="p-3 bg-blue-50 rounded-lg flex flex-col gap-2"
          >
              ...
              {editingId === task.id ? (
                ...
                <>
                  ...
                  {task.completed ? (
                    <button
                      onClick={() =>
                        handleToggleDone(task.id, task.title, task.category, false)
                      }
                    ...
                    <button
                      onClick={() =>
                        handleToggleDone(task.id, task.title, task.category, true)
                      }
